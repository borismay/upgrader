...
class SikluUnit:
    def __init__(self, host, username, password, port='22', connection_timeout=12):
        self.host = host
        self.user = username
    ...
    def connect(self):
        ...
        try:
            if platform.system() == 'Windows':
                foo = wexpect.spawn('plink %s@%s' % (self.user, self.host))
            else:  # assume linux
                foo = pexpect.spawn('ssh %s@%s' % (self.user, self.host))

            i = foo.expect([TIMEOUT, self.sshask_newkey, self.sshask_passwd, self.noroutehost], self.connection_timeout)
...
def run_command(unit_):
    ...
    unit.connect()
    if unit.connected:
        if 'upload_sw' in command:
            status = copy_sw_unit(unit, command)
        elif 'run_sw' in command:
...
def units_manager_parallel(cfg_file):
...
if __name__ == '__main__':
    ...
    units_manager_parallel(CSV_FILENAME)
